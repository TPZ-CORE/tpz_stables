...
AddEventHandler('tpz_stables:server:requestPlayerData', function()
	...
	exports["ghmattimysql"]:execute("SELECT `selected_horse_index` FROM `characters` WHERE `charidentifier` = @charidentifier", { ["@charidentifier"] = charIdentifier }, function(result)
		...
		if result and result[1] then
			selectedHorseIndex = result[1].selected_horse_index
		end

		TriggerClientEvent("tpz_stables:client:sendPlayerData", _source, Horses, Wagons, selectedHorseIndex )
...
Citizen.CreateThread(function()
	while true do
        ...
        if shouldSave then
            ...
            local length = GetTableLength(Horses)
            ...
            if length > 0 then
                ...
                for _, horse in pairs (Horses) do
					...
					exports.ghmattimysql:execute("UPDATE `horses` SET `name` = @name, `stats` = @stats, `components` = @components, `age` = @age, `isdead` = @isdead, `training_experience` = @training_experience, `breeding` = @breeding, `container` = @container WHERE id = @id", Parameters)
			...
			local length = GetTableLength(Wagons)
            ...
            if length > 0 then
                ...
                for _, wagon in pairs (Wagons) do
					...
					exports.ghmattimysql:execute("UPDATE `wagons` SET `name` = @name, `stats` = @stats, `components` = @components, `broken` = @broken, `container` = @container WHERE id = @id", Parameters)
