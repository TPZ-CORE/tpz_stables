...
AddEventHandler('tpz_stables:server:saveHorse', function(horseIndex, stamina, health, trainingExperience, remove)
	local _source = source
	local Horses  = GetHorses()
	...
	Horses[horseIndex].stats.training_experience = trainingExperience

	exports.ghmattimysql:execute("UPDATE `horses` SET `stats` = @stats, `training_experience` = @training_experience WHERE `id` = @id ", { 
		['id']                  = horseIndex, 
